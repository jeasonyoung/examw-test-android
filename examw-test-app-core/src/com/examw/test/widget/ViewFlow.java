...
public class ViewFlow extends AdapterView<Adapter> {
	...
	private enum LazyInit { LEFT, RIGHT }
		...
		switch (action) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (xMoved) {
					...
					if (mViewInitializeListener != null) initializeView(deltaX);
				}
	...
	private void setVisibleView(int indexInBuffer, boolean uiThread) {
		...
		if(dx == 0) onScrollChanged(mScroller.getCurrX() + dx, mScroller.getCurrY(), mScroller.getCurrX() + dx, mScroller.getCurrY());
		if (uiThread)
			invalidate();
		else
			postInvalidate();
	...
	private void logBuffer() {
		Log.d("viewflow", "Size of mLoadedViews: " + mLoadedViews.size() + ", Size of mRecycledViews: " + mRecycledViews.size() + ", X: " + mScroller.getCurrX() + ", Y: " + mScroller.getCurrY());
